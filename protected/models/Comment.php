	...
	public function getPendingCommentCount()
		...
		$comments=Comment::model()->with(array(
				...
				//		'joinType'=>'INNER JOIN',
						...
						//		't.status='.self::STATUS_PENDING),
						),
					))->findAll('t.status='.self::STATUS_PENDING);
		return count($comments);
//		return $this->count('status='.self::STATUS_PENDING.' and '.$this->allowed($a).'='.Yii::app()->user->id);
        }
	...
	{
		$model=Comment::model()->with('post')->findByPk($id);
		if($model->post->author_id==Yii::app()->user->id)
		return true;
		else return false;
//		if(strcmp($x->author_id,Yii::app()->user->id)==0)
//		return true;
//		else return false;
	}
